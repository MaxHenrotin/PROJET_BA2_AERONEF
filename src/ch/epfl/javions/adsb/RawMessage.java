 ...
 *
 * @param timeStampNs : l'horodatage du message, exprimé en nanosecondes depuis une origine donnée (généralement l'instant correspondant au tout premier échantillon de puissance calculé)
...
public record RawMessage(long timeStampNs, ByteString bytes) {
     ...
     * @param timeStampNs : l'horodatage du message, exprimé en nanosecondes depuis une origine donnée (généralement l'instant correspondant au tout premier échantillon de puissance calculé)
     * @param bytes : les octets du message ADS-B (dans l'ordre de transmission)
     * @throws IllegalArgumentException si l'horodatage est (strictement) négatif, ou si la chaîne d'octets ne contient pas LENGTH octets
     */
    public RawMessage{
    ...
    /**
     * Méthode statique retournant la taille d'un message dont le premier octet est celui donné (concrètement, retourne LENGTH si l'attribut DF contenu dans ce premier octet vaut 17, et 0 pour indiquer que le message est d'un type inconnu sinon)
     * @param byte0 : premier octet du message analysé
     * @return LENGTH si l'attribut DF contenu dans ce premier octet vaut 17, et 0 pour indiquer que le message est d'un type inconnu sinon
     */
    ...
    /**
     * Méthode publique retournant le code de type du message (c.-à-d. les cinq bits de poids le plus fort de son attribut ME)
